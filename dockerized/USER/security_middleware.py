...
# Purpose: OWASP Top 10 security middleware — security headers, rate-limiting,
#          audit logging, and SSRF endpoint validation.
# -*- coding: utf-8 -*-
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    - Auth endpoints (/login, /signup, /auth/token): 10 req/min per IP
    - All other endpoints: 200 req/min per IP
    """
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if path in self.SENSITIVE_PATHS or status_code >= 400:
            ...
            logger.log(
                ...
                request.method, path, status_code, ip,
...
# OWASP A10 — SSRF: validate RDS endpoint before making outbound calls
...
def validate_rds_endpoint(endpoint: str) -> str:
    """Validate that an RDS endpoint is a legitimate AWS hostname. OWASP A10:2021."""
